...
class Raid {
            ...
            .catch(err => {
                ...
                if (!!raid) {
                    ...
                    Object.values(this.raids)
                        ...
                        .forEach(raid => {
                            log.warn(`Deleting nonexistent message ${message_id} from raid ${raid.channel_id}`);
                            raid.messages.splice(raid.messages.indexOf(message_cache_id), 1);
    ...
    persistRaid(raid) {
        try {
            this.active_raid_storage.set(raid.channel_id, JSON.stringify(raid), 'EX', EXPIRE_TIME);
        } catch (err) {
            log.error(err);
        }
    }
    ...
    deleteRaid(channel_id, delete_channel = true) {
        const raid = this.getRaid(channel_id),
            getAsync = promisify(this.completed_raid_storage.get).bind(this.completed_raid_storage);
            ...
            .then(result => {
                ...
                getAsync(raid.gym_id.toString())
                    .then(gym_raids => {
                        ...
                        gym_raids.push(raid);
                        try {
                            this.completed_raid_storage.set(raid.gym_id.toString(), JSON.stringify(gym_raids), 'EX', EXPIRE_TIME)
                        } catch (err) {
                            log.error(err);
                        }
                        return true;
                    })
                    .then(result => this.active_raid_storage.del(channel_id))
                    .catch(err => log.error(err));
                            ...
                            .map(present_member => {
                                ...
                                return message.channel.awaitMessages(
                                    ...
                                    .then(collected_responses => {
                                        ...
                                        if (confirmation) {
                                            response.react(Helper.getEmoji(settings.emoji.thumbs_up) || '👍')
                                                .catch(err => log.error(err));
                                            ...
                                            response.react(Helper.getEmoji(settings.emoji.thumbs_down) || '👎')
                                                .catch(err => log.error(err));
